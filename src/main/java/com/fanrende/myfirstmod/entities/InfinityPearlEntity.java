...
public class InfinityPearlEntity extends ProjectileItemEntity
{
	private LivingEntity perlThrower;

	public InfinityPearlEntity(EntityType<? extends InfinityPearlEntity> p_i50153_1_, World p_i50153_2_)
	{
	...
	protected void onImpact(RayTraceResult result)
	{
		...
		if (result.getType() == RayTraceResult.Type.BLOCK)
		{
			...
			if (tileentity instanceof EndGatewayTileEntity)
			{
				...
				if (livingentity != null)
				{
					...
					{
						CriteriaTriggers.ENTER_BLOCK.trigger((ServerPlayerEntity) livingentity,
		...
		if (!this.world.isRemote)
		{
			if (livingentity instanceof ServerPlayerEntity)
			{
				...
				{
					net.minecraftforge.event.entity.living.EnderTeleportEvent event = new net.minecraftforge.event.entity.living.EnderTeleportEvent(serverplayerentity,
							this.getPosX(),
							this.getPosY(),
							this.getPosZ(),
							5.0F
					...
					{ // Don't indent to lower patch size
								...
								.getBoolean(GameRules.DO_MOB_SPAWNING))
						{
							...
							endermiteentity.setLocationAndAngles(livingentity.getPosX(),
